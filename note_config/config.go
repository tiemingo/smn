...
type Config struct {
	...
	// Encryption
	UseEncryption bool `yaml:"use_encryption,omitempty"` // Encrypts files, add note.md to gitignore

	// Defaults

	Template     string   `yaml:"template,omitempty"`      // Path to template
	Authors      []string `yaml:"authors,omitempty"`       // List of default authors
	BuildMode    string   `yaml:"build_mode,omitempty"`    // Default build mode
	GitIgnoreOut bool     `yaml:"gitignore_out,omitempty"` // Should add .out folder to notes dir when creating new note
	...
	BuildReplaceSpace string `yaml:"build_replace_space,omitempty"` // With what a space in the output filename should be replaces use " " to leave the space.
